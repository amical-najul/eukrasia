...
export const StatusCircle = ({ statusData }) => {
    ...
    // Logic: Always "Fasting" timer since last meal.
    // We visually represent phases.
    ...
    // Always pulse if significant time elapsed? Or just static. Let's pulse if > 12h.
        ...
        <div className="flex flex-col items-center justify-center py-8">
            <div className={`relative w-64 h-64 rounded-full border-8 ${borderColor} flex flex-col items-center justify-center bg-gray-900 ${ringPulse} transition-all duration-500`}>
                ...
                <div className={`text-5xl font-black ${textColor} tabular-nums`}>
                    {hours_elapsed ? hours_elapsed.toFixed(1) : '0.0'} <span className="text-xl">h</span>
                </div>
            ...
            {/* Removed Manual Button as per user request for auto-logic */}
    ...
    <div className="bg-transparent">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
            {items.map((item, idx) => (
                <button
                    key={idx}
                    onClick={() => infoMode ? onInfo(item) : onLog(item, category, breaker)}
...
// 2. Camera Modal (Photo Required, No Notes)
export const CameraModal = ({ isOpen, onClose, initialItem, onConfirm }) => {
    ...
    const handleSubmit = () => {
        if (!image) {
            alert("Por favor toma una foto para registrar este alimento.");
            return;
        }
        onConfirm({ ...initialItem, image });
        setImage(null);
        setPreview(null);
        onClose();
    };
            ...
            <div className="bg-gray-900 w-full max-w-md rounded-3xl border border-gray-700 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900/50">
                    <h3 className="text-white font-bold flex items-center gap-2">
                        <Camera size={20} className="text-indigo-400" /> Registro Visual
                    </h3>
                ...
                <div className="p-6 space-y-6 overflow-y-auto">
                        ...
                        <div className={`w-full aspect-square rounded-2xl border-2 border-dashed flex flex-col items-center justify-center transition-all duration-300
                            ...
                            {preview ? (
                                ...
                                <>
                                    ...
                                    <span className="text-sm text-gray-400 font-medium group-hover:text-indigo-300">Tocar para FOTO OBLIGATORIA</span>
                                </>
                            )}
                        </div>
                    </label>
                    ...
                    <button
                        ...
                        className={`w-full py-4 rounded-xl font-black tracking-wide shadow-lg transition-all active:scale-95 flex justify-center items-center gap-2
                            ${image ? 'bg-indigo-600 hover:bg-indigo-500 text-white' : 'bg-gray-700 text-gray-500 cursor-not-allowed'}
                        `}
                    >
